    ...
    switch ($method) {
        case 'GET':
            ...
            if (isset($_GET['today-score']) || strpos($_SERVER['QUERY_STRING'], 'today-score') !== false) {
                ...
                // ดึงคะแนนวันนี้จาก vw_BehaviorDailySummary
                $today = date('Y-m-d');
                ...
                $sql = "
                    SELECT 
                        c.Id,
                        c.Name,
                        COALESCE(SUM(v.TotalPoints * v.TotalCount), 0) as TodayScore,
                        COUNT(CASE WHEN v.Type = 'Good' AND v.TotalCount > 0 THEN 1 END) as TodayGoodBehaviors,
                        COUNT(CASE WHEN v.Type = 'Bad' AND v.TotalCount > 0 THEN 1 END) as TodayBadBehaviors,
                        COUNT(CASE WHEN v.TotalCount > 0 THEN 1 END) as TotalActivities
                    FROM Children c
                    LEFT JOIN vw_BehaviorDailySummary v ON c.Id = v.ChildId 
                        AND v.ActivityDate = ?
                ...
                if ($stmt === false) {
                    // ถ้า view ไม่มี ลอง DailyActivity
                    $fallbackSql = "
                        SELECT 
                            c.Id,
                            c.Name,
                            COALESCE(SUM(da.TotalPoints), 0) as TodayScore,
                            COUNT(CASE WHEN b.Type = 'Good' AND da.Count > 0 THEN 1 END) as TodayGoodBehaviors,
                            COUNT(CASE WHEN b.Type = 'Bad' AND da.Count > 0 THEN 1 END) as TodayBadBehaviors,
                            COUNT(CASE WHEN da.Count > 0 THEN 1 END) as TotalActivities
                        FROM Children c
                        LEFT JOIN DailyActivity da ON c.Id = da.ChildId AND da.ActivityDate = ?
                        LEFT JOIN Behaviors b ON da.BehaviorId = b.Id
                        WHERE c.Id = ? AND c.IsActive = 1
                        GROUP BY c.Id, c.Name
                    ";
                    
                    $stmt = sqlsrv_query($conn, $fallbackSql, [$today, $id]);
                    
                    if ($stmt === false) {
                        // ถ้ายังไม่ได้ ลอง ActivityLogs
                        $lastFallbackSql = "
                            SELECT 
                                c.Id,
                                c.Name,
                        ...
                        $stmt = sqlsrv_query($conn, $lastFallbackSql, [$today, $id]);
                        
                        if ($stmt === false) {
                            throw new Exception('All today score queries failed: ' . print_r(sqlsrv_errors(), true));
                        }
                    }
                }
                ...
                sendJson([
                    ...
                    'date' => $today,
                    'totalScore' => (int)($result['TodayScore'] ?? 0),
                    'todayActivities' => [
                        'goodBehaviors' => (int)($result['TodayGoodBehaviors'] ?? 0),
                        'badBehaviors' => (int)($result['TodayBadBehaviors'] ?? 0),
            ...
            if ($id) {
                // Get specific child with calculated points - ใช้ DailyActivity หรือ ActivityLogs
                $sql = "
                    SELECT 
                ...
                if ($stmt === false) {
                    throw new Exception('Child query failed: ' . print_r(sqlsrv_errors(), true));
                }
        ...
        case 'POST':
            // Handle creating new child (เหมือนเดิม)
...
function handleDashboard($conn, $method) {
    ...
    try {
        ...
        // Get today's summary by child from DailyActivity if available
        $todaySummaryByChild = [];
        try {
            $todaySummarySql = "
                SELECT 
            ...
            // DailyActivity table might not exist
            $todaySummaryByChild = [];
        }
        ...
        // Behavior statistics from view
        $totalBehaviors = count($behaviorSummary);
        $activeBehaviors = count(array_filter($behaviorSummary, fn($b) => $b['isCompleted']));
        $goodBehaviors = count(array_filter($behaviorSummary, fn($b) => $b['Type'] === 'Good'));
        $badBehaviors = count(array_filter($behaviorSummary, fn($b) => $b['Type'] === 'Bad'));
        ...
        sendJson([
            ...
            'data_sources' => [
                ...
                'behavior_summary' => 'vw_BehaviorDailySummary',
                'today_activities' => 'ActivityLogs',
                'today_summary' => 'DailyActivity'
            ]
        ]);
